...
function () {
	...
	function createTemplate_default(callback){
		...
		$.ajax({url: contextPath + "/templates/deploy/history/right_default.mst", dataType: 'text'})
	...
	function createTemplate(data, callback){
		...
		var container = $('#row_main_right');
		$.ajax({url: contextPath + "/templates/deploy/history/right.mst", dataType: 'text'})
		.done(function(template){
			Mustache.parse(template);
	...
	function createTable(){
		...
		$('#contentHistoryListTable').DataTable( {
			destroy: true,
	        paging: true,
	        bAutoWidth: false,
 			searching: false,
	        ...
	        "ajax": {
	        	type: "GET",
	            url: contextPath + "/data/sample_export_history_detail_list.json",
	            dataType: 'json',
	            dataSrc: "data",
	            cache:false
	        },
